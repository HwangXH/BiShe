...
def squeeze_excite_block(input_data, input_channel, filter_num1, filter_num2, trainable, name):
    ...
    with tf.variable_scope(name):
        ...
        squeeze = tf.keras.layers.GlobalAveragePooling2D()(input_data)
        excitation = tf.layers.dense(squeeze, units=filter_num2 / 16, activation='relu')
        excitation = tf.layers.dense(excitation, units=filter_num2, activation='sigmoid')
        # input_data = tf.keras.layers.Dense(filter_num2 / 16, activation='relu')
        # input_data = tf.keras.layers.Dense(filter_num2, activation='sigmoid')
        scale = tf.keras.layers.multiply([input_data, excitation])
        se_output = scale + short_cut
