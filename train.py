...
class YoloTrain(object):
    ...
    def train(self):
        ...
        # 大气光A，取暗通道的前千分之一最亮（灰度值最大）的像素点，在原图中找到这些像素点，取其中最大亮度值
        def AtmLight(im, dark):
            [h, w] = im.shape[:2]
            imsz = h * w
            numpx = int(max(math.floor(imsz / 1000), 1))
            darkvec = dark.reshape(imsz, 1) # 暗通道只有灰色，原图是rgb一共3通道
            ...
            A = atmsum / numpx  # 1x3 的向量， 取1000个像素点的亮度取平均
            return A

        def DarkIcA(im, A):
            im3 = np.empty(im.shape, im.dtype)  # 创建一个空数组，数据将近为0
                ...
                if args.fog_FLAG:
                    ...
                    if DefogFilter in cfg.filters:
                        ...
                        for i in range(train_data[0].shape[0]):
                            dark_i = DarkChannel(train_data[0][i]) # train_data[0][i]是一张图像的局部，获得暗通道下的图像
                            defog_A_i = AtmLight(train_data[0][i], dark_i) # 原图和暗通道图像，获得大气光参数
